Create Codex pet row `running-right` for `yangmi-green-bee`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A petite, pet-safe chibi mascot inspired by the supplied portrait: dark updo, green floral dress, tiny bee wings, green and honey-yellow palette; no text or logos.. Preserve silhouette, face, palette, material, proportions, markings, and props.
