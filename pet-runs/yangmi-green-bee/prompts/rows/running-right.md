Create one horizontal animation strip for Codex pet `yangmi-green-bee`, state `running-right`.
...
Identity: same pet in every frame: A petite, pet-safe chibi mascot inspired by the supplied portrait: dark updo, green floral dress, tiny bee wings, green and honey-yellow palette; no text or logos.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `sticker`: Polished sticker mascot with bold clean shapes, crisp outline, flat colors, and minimal highlight detail. User style notes: Cute full-body sticker mascot, crisp outline, clean silhouette, no scenery..
